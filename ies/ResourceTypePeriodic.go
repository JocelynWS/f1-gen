...
type ResourceTypePeriodic struct {
	Periodicity ResourceTypePeriodicPeriodicity `madatory,valueExt`
	Offset      int64                           `lb:0,ub:2559,madatory,valueExt`
	// IEExtensions *ResourceTypePeriodicExtIEs `optional`
}

func (ie *ResourceTypePeriodic) Encode(w *aper.AperWriter) (err error) {
	...
	tmp_Offset := NewINTEGER(ie.Offset, aper.Constraint{Lb: 0, Ub: 2559}, true)
...
func (ie *ResourceTypePeriodic) Decode(r *aper.AperReader) (err error) {
	...
	tmp_Offset := INTEGER{c: aper.Constraint{Lb: 0, Ub: 2559}, ext: true}
