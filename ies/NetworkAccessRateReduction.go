...
type NetworkAccessRateReduction struct {
	TransactionID     int64             `lb:0,ub:255,mandatory,reject`
	UACAssistanceInfo UACAssistanceInfo `mandatory,reject`
}
...
func (msg *NetworkAccessRateReduction) toIes() (ies []F1apMessageIE, err error) {
	...
	ies = append(ies, F1apMessageIE{
		Id:          ProtocolIEID{Value: ProtocolIEID_UACAssistanceInfo},
		Criticality: Criticality{Value: Criticality_PresentReject},
		Value:       &msg.UACAssistanceInfo,
	})
	return
}
func (msg *NetworkAccessRateReduction) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("NetworkAccessRateReduction"), err)
		}
	...
	r.ReadBool()
	decoder := NetworkAccessRateReductionDecoder{
		msg:  msg,
	...
	if _, ok := decoder.list[ProtocolIEID_UACAssistanceInfo]; !ok {
		err = fmt.Errorf("Mandatory field UACAssistanceInfo is missing")
		decoder.diagList = append(decoder.diagList, CriticalityDiagnosticsIEItem{
			IECriticality: Criticality{Value: Criticality_PresentReject},
			IEID:          ProtocolIEID{Value: ProtocolIEID_UACAssistanceInfo},
			TypeOfError:   TypeOfError{Value: TypeOfErrorMissing},
		})
		return
	}
...
type NetworkAccessRateReductionDecoder struct {
	msg      *NetworkAccessRateReduction
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*F1apMessageIE
}

func (decoder *NetworkAccessRateReductionDecoder) decodeIE(r *aper.AperReader) (msgIe *F1apMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
	...
	case ProtocolIEID_UACAssistanceInfo:
		var tmp UACAssistanceInfo
		if err = tmp.Decode(ieR); err != nil {
			err = utils.WrapError("Read UACAssistanceInfo", err)
			return
		}
		msg.UACAssistanceInfo = tmp
	default:
