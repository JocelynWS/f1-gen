...
func (msg *ECIDMeasurementInitiationFailure) Encode(w io.Writer) (err error) {
	...
	if ies, err = msg.toIes(); err != nil {
		err = msgErrors(fmt.Errorf("ECIDMeasurementInitiationFailure"), err)
		return
	}
...
func (msg *ECIDMeasurementInitiationFailure) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	...
	r.ReadBool()
	decoder := ECIDMeasurementInitiationFailureDecoder{
		msg:  msg,
...
type ECIDMeasurementInitiationFailureDecoder struct {
	msg      *ECIDMeasurementInitiationFailure
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*F1apMessageIE
}

func (decoder *ECIDMeasurementInitiationFailureDecoder) decodeIE(r *aper.AperReader) (msgIe *F1apMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
