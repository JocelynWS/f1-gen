...
func (msg *ECIDMeasurementFailureIndication) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("ECIDMeasurementFailureIndication"), err)
		}
	...
	r.ReadBool()
	decoder := ECIDMeasurementFailureIndicationDecoder{
		msg:  msg,
...
type ECIDMeasurementFailureIndicationDecoder struct {
	msg      *ECIDMeasurementFailureIndication
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*F1apMessageIE
}

func (decoder *ECIDMeasurementFailureIndicationDecoder) decodeIE(r *aper.AperReader) (msgIe *F1apMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
