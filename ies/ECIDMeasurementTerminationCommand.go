...
func (msg *ECIDMeasurementTerminationCommand) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("ECIDMeasurementTerminationCommand"), err)
		}
	...
	r.ReadBool()
	decoder := ECIDMeasurementTerminationCommandDecoder{
		msg:  msg,
...
type ECIDMeasurementTerminationCommandDecoder struct {
	msg      *ECIDMeasurementTerminationCommand
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*F1apMessageIE
}

func (decoder *ECIDMeasurementTerminationCommandDecoder) decodeIE(r *aper.AperReader) (msgIe *F1apMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
