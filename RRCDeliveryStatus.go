...
type RRCDeliveryStatus struct {
	...
	RRCDeliveryStatusExtIEs F1APPROTOCOLEXTENSION `mandatory`
}
...
func (ie *RRCDeliveryStatus) Encode(w *aper.AperWriter) (err error) {
	...
	if err = ie.RRCDeliveryStatusExtIEs.Encode(w); err != nil {
		err = utils.WrapError("Encode RRCDeliveryStatusExtIEs", err)
		return
	}
	return
}
func (ie *RRCDeliveryStatus) Decode(r *aper.AperReader) (err error) {
	...
	if err = ie.RRCDeliveryStatusExtIEs.Decode(r); err != nil {
		err = utils.WrapError("Read RRCDeliveryStatusExtIEs", err)
		return
	}
	return
}
