...
func (msg *PositioningInformationUpdate) Encode(w io.Writer) (err error) {
    ...
    if ies, err = msg.toIes(); err != nil {
        err = msgErrors(fmt.Errorf("PositioningInformationUpdate"), err)
        return
    }
...
func (msg *PositioningInformationUpdate) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	...
	r.ReadBool()
	decoder := PositioningInformationUpdateDecoder{
		msg:  msg,
...
type PositioningInformationUpdateDecoder struct {
	msg      *PositioningInformationUpdate
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*F1apMessageIE
}

func (decoder *PositioningInformationUpdateDecoder) decodeIE(r *aper.AperReader) (msgIe *F1apMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
