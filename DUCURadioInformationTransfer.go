...
type DUCURadioInformationTransfer struct {
	TransactionID            int64                    `lb:0,ub:255,mandatory,reject`
	DUCURadioInformationType DUCURadioInformationType `mandatory,ignore`
}
...
func (msg *DUCURadioInformationTransfer) toIes() (ies []F1apMessageIE, err error) {
	...
	ies = append(ies, F1apMessageIE{
		Id:          ProtocolIEID{Value: ProtocolIEID_DUCURadioInformationType},
		Criticality: Criticality{Value: Criticality_PresentIgnore},
		Value:       &msg.DUCURadioInformationType,
	})
	return
}
func (msg *DUCURadioInformationTransfer) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("DUCURadioInformationTransfer"), err)
		}
	...
	r.ReadBool()
	decoder := DUCURadioInformationTransferDecoder{
		msg:  msg,
	...
	if _, ok := decoder.list[ProtocolIEID_DUCURadioInformationType]; !ok {
		err = fmt.Errorf("Mandatory field DUCURadioInformationType is missing")
		decoder.diagList = append(decoder.diagList, CriticalityDiagnosticsIEItem{
			IECriticality: Criticality{Value: Criticality_PresentIgnore},
			IEID:          ProtocolIEID{Value: ProtocolIEID_DUCURadioInformationType},
			TypeOfError:   TypeOfError{Value: TypeOfErrorMissing},
		})
		return
	}
...
type DUCURadioInformationTransferDecoder struct {
	msg      *DUCURadioInformationTransfer
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*F1apMessageIE
}

func (decoder *DUCURadioInformationTransferDecoder) decodeIE(r *aper.AperReader) (msgIe *F1apMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
	...
	case ProtocolIEID_DUCURadioInformationType:
		var tmp DUCURadioInformationType
		if err = tmp.Decode(ieR); err != nil {
			err = utils.WrapError("Read DUCURadioInformationType", err)
			return
		}
		msg.DUCURadioInformationType = tmp
	default:
