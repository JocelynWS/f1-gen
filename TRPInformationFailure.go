...
func (msg *TRPInformationFailure) Encode(w io.Writer) (err error) {
	var ies []F1apMessageIE
	if ies, err = msg.toIes(); err != nil {
		err = msgErrors(fmt.Errorf("TRPInformationFailure"), err)
		return
	}
	return encodeMessage(w, F1apPduUnsuccessfulOutcome, ProcedureCode_TRPInformationExchange, Criticality_PresentReject, ies)
}
func (msg *TRPInformationFailure) toIes() (ies []F1apMessageIE, err error) {
	ies = []F1apMessageIE{}
...
func (msg *TRPInformationFailure) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("TRPInformationFailure"), err)
		}
	...
	r.ReadBool()
	decoder := TRPInformationFailureDecoder{
		msg:  msg,
...
type TRPInformationFailureDecoder struct {
	msg      *TRPInformationFailure
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*F1apMessageIE
}

func (decoder *TRPInformationFailureDecoder) decodeIE(r *aper.AperReader) (msgIe *F1apMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
