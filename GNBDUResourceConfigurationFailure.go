...
type GNBDUResourceConfigurationFailure struct {
...
func (msg *GNBDUResourceConfigurationFailure) Encode(w io.Writer) (err error) {
    var ies []F1apMessageIE
    if ies, err = msg.toIes(); err != nil {
        err = msgErrors(fmt.Errorf("GNBDUResourceConfigurationFailure"), err)
        return
    }
    return encodeMessage(w, F1apPduUnsuccessfulOutcome, ProcedureCode_GNBDUResourceConfiguration, Criticality_PresentReject, ies)
}
func (msg *GNBDUResourceConfigurationFailure) toIes() (ies []F1apMessageIE, err error) {
	ies = []F1apMessageIE{}
...
func (msg *GNBDUResourceConfigurationFailure) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("GNBDUResourceConfigurationFailure"), err)
		}
	...
	r.ReadBool()
	decoder := GNBDUResourceConfigurationFailureDecoder{
		msg:  msg,
...
type GNBDUResourceConfigurationFailureDecoder struct {
	msg      *GNBDUResourceConfigurationFailure
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*F1apMessageIE
}

func (decoder *GNBDUResourceConfigurationFailureDecoder) decodeIE(r *aper.AperReader) (msgIe *F1apMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
