...
type RANUEPagingIdentity struct {
	...
	RANUEPagingIdentityExtIEs F1APPROTOCOLEXTENSION `mandatory`
}
...
func (ie *RANUEPagingIdentity) Encode(w *aper.AperWriter) (err error) {
	...
	if err = ie.RANUEPagingIdentityExtIEs.Encode(w); err != nil {
		err = utils.WrapError("Encode RANUEPagingIdentityExtIEs", err)
		return
	}
	return
}
func (ie *RANUEPagingIdentity) Decode(r *aper.AperReader) (err error) {
	...
	ie.IRNTI = aper.BitString{Bytes: tmp_IRNTI.Value.Bytes, NumBits: tmp_IRNTI.Value.NumBits}
	if err = ie.RANUEPagingIdentityExtIEs.Decode(r); err != nil {
		err = utils.WrapError("Read RANUEPagingIdentityExtIEs", err)
		return
	}
	return
}
