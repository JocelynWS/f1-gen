...
type BAPMappingConfigurationFailure struct {
...
func (msg *BAPMappingConfigurationFailure) Encode(w io.Writer) (err error) {
    var ies []F1apMessageIE
    if ies, err = msg.toIes(); err != nil {
        err = msgErrors(fmt.Errorf("BAPMappingConfigurationFailure"), err)
        return
    }
    return encodeMessage(w, F1apPduUnsuccessfulOutcome, ProcedureCode_BAPMappingConfiguration, Criticality_PresentReject, ies)
}
func (msg *BAPMappingConfigurationFailure) toIes() (ies []F1apMessageIE, err error) {
	ies = []F1apMessageIE{}
...
func (msg *BAPMappingConfigurationFailure) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("BAPMappingConfigurationFailure"), err)
		}
	...
	r.ReadBool()
	decoder := BAPMappingConfigurationFailureDecoder{
		msg:  msg,
...
type BAPMappingConfigurationFailureDecoder struct {
	msg      *BAPMappingConfigurationFailure
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*F1apMessageIE
}

func (decoder *BAPMappingConfigurationFailureDecoder) decodeIE(r *aper.AperReader) (msgIe *F1apMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
