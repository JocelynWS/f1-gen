...
type PositioningDeactivation struct {
	...
	AbortTransmission AbortTransmission `mandatory,ignore`
}
...
func (msg *PositioningDeactivation) toIes() (ies []F1apMessageIE, err error) {
	...
	ies = append(ies, F1apMessageIE{
		Id:          ProtocolIEID{Value: ProtocolIEID_AbortTransmission},
		Criticality: Criticality{Value: Criticality_PresentIgnore},
		Value:       &msg.AbortTransmission,
	})
	return
}
func (msg *PositioningDeactivation) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("PositioningDeactivation"), err)
		}
	...
	r.ReadBool()
	decoder := PositioningDeactivationDecoder{
		msg:  msg,
	...
	if _, ok := decoder.list[ProtocolIEID_AbortTransmission]; !ok {
		err = fmt.Errorf("Mandatory field AbortTransmission is missing")
		decoder.diagList = append(decoder.diagList, CriticalityDiagnosticsIEItem{
			IECriticality: Criticality{Value: Criticality_PresentIgnore},
			IEID:          ProtocolIEID{Value: ProtocolIEID_AbortTransmission},
			TypeOfError:   TypeOfError{Value: TypeOfErrorMissing},
		})
		return
	}
...
type PositioningDeactivationDecoder struct {
	msg      *PositioningDeactivation
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*F1apMessageIE
}

func (decoder *PositioningDeactivationDecoder) decodeIE(r *aper.AperReader) (msgIe *F1apMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
	...
	case ProtocolIEID_AbortTransmission:
		var tmp AbortTransmission
		if err = tmp.Decode(ieR); err != nil {
			err = utils.WrapError("Read AbortTransmission", err)
			return
		}
		msg.AbortTransmission = tmp
	default:
